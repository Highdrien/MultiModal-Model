...
class Wav2Vec2Classifier(BaseModel):
    def __init__(
        ...
        audio_length_2_hidden_size = {1000: 4096, 2000: 12288, 3000: 18432}
        ...
        self.fc = torch.nn.Linear(
            in_features=audio_length_2_hidden_size[audio_length],
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x0 = self.processor(
            x0, return_tensors="pt", padding="longest", sampling_rate=16000
        ).input_values.to(self.model.device)
        x1 = self.processor(
            x1, return_tensors="pt", padding="longest", sampling_rate=16000
        ).input_values.to(self.model.device)
...
if __name__ == "__main__":
    ...
    model = Wav2Vec2Classifier(
        last_layer=True, num_classes=2, audio_length=audio_length
    )
    model.train()
