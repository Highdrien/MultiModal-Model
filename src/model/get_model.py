...
def get_model(config: EasyDict) -> Model:
    ...
    if config.task not in implemented:
        raise NotImplementedError(
            f"Expected config.task in {implemented} but found {config.task}"
        )
    ...
    if config.task == "audio":
        model = wave2vec.Wav2Vec2Classifier(
            pretrained_model_name=cfg_model.pretrained_model_name,
            last_layer=True,
            num_classes=config.data.num_classes,
    ...
    if config.task == "multi":
        ...
        if config.load.text[0]:
            ...
            text = bert.BertClassifier(
                hidden_size=text_config.model.text.hidden_size,
        ...
        if config.load.audio[0]:
            ...
            audio = wave2vec.Wav2Vec2Classifier(
                pretrained_model_name=audio_config.model.audio.pretrained_model_name,
                last_layer=True,
        ...
        if config.load.video[0]:
            ...
            video = lstm.LSTMClassifier(
                num_features=video_config.data.num_features,
        ...
        if config.model.multi.likelihood:
            ...
            model = multimodal.MultimodalClassifier(
                basemodel=basemodel,
