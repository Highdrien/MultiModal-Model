...
def train(config: EasyDict) -> None:
    ...
    scheduler = MultiStepLR(
        optimizer, milestones=config.learning.milesstone, gamma=config.learning.gamma
    )
    ...
    for epoch in range(1, config.learning.epochs + 1):
        ...
        for i, (data, y_true) in enumerate(train_range):
            ...
            train_range.set_description(
                f"TRAIN -> epoch: {epoch} || loss: {current_loss:.4f}"
            )
            train_range.refresh()
        ...
        with torch.no_grad():
            ...
            for i, (data, y_true) in enumerate(val_range):
                ...
                val_range.set_description(
                    f"VAL   -> epoch: {epoch} || loss: {current_loss:.4f}"
                )
                val_range.refresh()
        ...
        if save_experiment:
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(
                    model.get_only_learned_parameters(),
    ...
    stop_time = time.time()
    print(
        f"training time: {stop_time - start_time}secondes for {config.learning.epochs} epochs"
    )
